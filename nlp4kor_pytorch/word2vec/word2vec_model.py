...
class SGNSModel(torch.nn.Module):
    ...
    def __init__(self, vocab_size, embedding_size, vocab: 'Word2VecVocab', neg_sample=20, padding_idx=0, neg_weight=True):
        ...
        log.debug(f'SGNSModel(vocab_size: {NumUtil.comma_str(self.vocab_size)}, n_negs: {self.n_negs})')
    ...
    def forward(self, iword, owords):
        ...
        if self.ns_weights is not None:
            nwords = torch.multinomial(self.ns_weights, batch_size * context_size * self.n_negs, replacement=True).view(batch_size, -1)  # BCN -> Bx(CN)
        else:
            nwords = torch.FloatTensor(batch_size, context_size * self.n_negs).uniform_(0, self.vocab_size - 1).long()  # Bx(CN)
        ...
        nloss = torch.bmm(nvectors, ivectors).sigmoid().log().view(-1, context_size, self.n_negs).sum(dim=2).mean(dim=1)  # Bx(CN) -> Bx(CxN) -> sum -> Bx(C) -> mean -> B
